...
namespace ARElements
{
    public class AnnotationScreenCard : MonoBehaviour
    {
        ...
        {
            scrollRect.onValueChanged.AddListener(new UnityAction<Vector2>(ScrollVerticalPositionChanged));
        }

        private void OnDisable()
        {
            scrollRect.onValueChanged.RemoveListener(new UnityAction<Vector2>(ScrollVerticalPositionChanged));
        }
        ...
        {
            if (cardState != ScreenCardState.Visible)
            {
                return;
            }
            if (scrollRect.normalizedPosition.y > dismissPushThreshold)
            {
                HideCard();
            }
        }
        ...
        public Action<AnnotationScreenCard.ScreenCardState> onCardStateChanged;
